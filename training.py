...
class TrainManager:
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> Tensor:
        ...
        batch_loss, noise = self.model.get_loss_for_batch(is_train=True,
                                                          batch=batch,
                                                          loss_function=self.loss)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Sign-KID")
    parser.add_argument("--config", default="./Configs/TSG0627.yaml", type=str, help="path to YAML config file")
